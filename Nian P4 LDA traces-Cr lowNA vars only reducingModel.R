...
data2f <- na.omit(cbind(nx.clr[c("Type.S3","Redox","Site","Depth.cm","Ba.ms", 
          "Cr.ms", "Pb.ms", "Sr.ms", "Th.ms", "Ti.ms", "U.ms")]))
data2.alr <- alr(data2, j=2)
colnames(data2.alr) <- c("Ba", "Pb", "Sr","Th", "Ti", "U")
...
attach(data2f.alr)
########## RUN LDA ##########
lda.nx.alr <- lda(formula = Type.S3~Ba+ Pb+ Sr+ Th+ Ti+ U, data=data2f.alr, prior=c(1,1,1)/3)
...
lda.nx.alr.values <- predict(lda.nx.alr, 
                             data2f.alr[c("Ba", "Pb", "Sr","Th", "Ti", "U")], na.rm=T)
