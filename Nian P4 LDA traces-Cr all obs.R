...
data2.alr <- alr(data2, j=5)
colnames(data2.alr) <- c("As","Ba", "Cd", "Co", "Cu", "Fe", "Mn", "Mo", "Ni","P", "Pb", "Rb", "S", 
                         "Sr","Th", "Ti", "U",  "V",  "Zn", "REE")
...
attach(data2f.alr)
########## RUN LDA ##########
lda.nx.alr <- lda(formula = Type.S3~As+Ba+ Cd+ Co+ Cu+ Fe+ 
                      Mn+ Mo+ Ni+P+ Pb+ Rb+ S+ 
...
lda.nx.alr.values <- predict(lda.nx.alr, data2f.alr[c("As","Ba", "Cd", "Co", "Cu", "Fe", "Mn", "Mo", "Ni","P", "Pb", "Rb", "S", 
...
plot(lda.nx.alr$scaling[,1],lda.nx.alr$scaling[,2],col="skyblue4",pch=18,lwd=1,cex=1.4,
     ...
     ylab="Linear Discriminant [2]", xlim=c(-9,15), 
